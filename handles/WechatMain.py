...
class wechat(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if signature != 'default' and timestamp != 'default' and nonce != 'default' and echostr != 'default' \
                and check_signature(wechatsettings['token'], signature, timestamp, nonce):
            self.write(echostr)
        # 处理其他莫名其妙的get请求。。。。
        else:
    ...
    def post(self):
        ...
        if signature != 'default' and timestamp != 'default' and nonce != 'default' \
            ...
            # 解析明文xml消息
            if wechatsettings['encrypt_mode'] == 'normal':
                msg = parse_message(body)
            # 解析加密xml消息
            elif wechatsettings['encrypt_mode'] == 'aes':
                crypto = WeChatCrypto(wechatsettings['token'], wechatsettings['encoding_aes_key'], wechatsettings['appid'])
                try:
